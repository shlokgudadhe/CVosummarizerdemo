...
class ConversationSummarizer:
    ...
    def get_context_for_api(self, current_session, latest_messages, gemini_api_key):
        ...
        # Add current chunk summary (if it exists)
        if current_session and current_session.get("current_chunk_summary"):
            context_parts.append("=== CURRENT CHUNK SUMMARY ===")
            context_parts.append(f"Current chunk summary: {current_session['current_chunk_summary']}")
        
        # Add non-latest messages from current chunk
        if current_session and current_session.get("current_chunk_messages"):
            # Get all messages except the latest ones
            all_chunk_messages = current_session["current_chunk_messages"]
            latest_count = len(latest_messages) if latest_messages else 0
            
            if len(all_chunk_messages) > latest_count:
                context_parts.append("=== EARLIER IN CURRENT CHUNK ===")
                earlier_messages = all_chunk_messages[:-latest_count] if latest_count > 0 else all_chunk_messages
        ...
        # Add recent conversation context (latest messages)
...
with st.sidebar:
    ...
    # Add this to your sidebar section, after the existing debug section
     
    st.header("Data Management")

    # Download current pkl file
    if os.path.exists(summarizer.summary_file):
        with open(summarizer.summary_file, 'rb') as f:
            pkl_data = f.read()
        
        st.download_button(
            label="📥 Download Conversation History",
            data=pkl_data,
            file_name=f"conversation_summaries_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl",
            mime="application/octet-stream",
            help="Download your conversation summaries as a pickle file"
        )
    else:
        st.info("No conversation history file found yet.")

    # Optional: Add file upload to restore summaries
    uploaded_file = st.file_uploader(
        "📤 Upload Previous History", 
        type=['pkl'],
        help="Upload a previously downloaded conversation history file"
    )

    if uploaded_file is not None:
        try:
            # Load the uploaded file
            uploaded_data = pickle.load(uploaded_file)
            
            # Validate it's the right format (list of session dictionaries)
            if isinstance(uploaded_data, list):
                summarizer.saved_summaries = uploaded_data
                summarizer.save_summaries()
                st.success("✅ Conversation history restored successfully!")
            else:
                st.error("❌ Invalid file format. Please upload a valid conversation history file.")
        except Exception as e:
